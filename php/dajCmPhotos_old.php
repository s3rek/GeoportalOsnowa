...
$ext = '.jpg';
  ...
  $result = pg_query($conn, 'SELECT id, miejsce_id, photo_nr, photo_opis FROM ciekawe_miejsca_photos WHERE miejsce_id='.$id);
...
if (pg_num_rows($result) > 0)
{
	...
	while ($row = pg_fetch_row($result))
	{
			...
			foreach ($files as $file)
			{
			   ...
			   $html = $html.'<a target="_blank" href="'.$dir.basename($file).'">';
			   $html = $html.'<p><center><img src="'.$dir.'//thumb//'.basename($file).'"></center></p></a>';
